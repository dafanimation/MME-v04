...
// frontend/src/pages/MiActividad.jsx
...
import { ImageCropUploader } from '../components/activities/ImageCropUploader'  // ✅ import añadido
...
const BipValidationPanel = ({ udCode, onUdChange }) => {
  // ... (código del panel BIP sin cambios) ...
  // (por brevedad se omite, pero mantén el mismo código que tenías)
  // Asegúrate de que todas las funciones y estilos estén presentes
  return <div>Panel BIP</div> // placeholder – reemplazar con el código original
}
      ...
      <div style={{ padding: '20px' }}>
        ...
        {bipMode ? null : (<>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
            <button onClick={() => setStateFilter('pending')} style={{ padding: '6px 10px', borderRadius: '8px', border: '1px solid #395579', background: stateFilter === 'pending' ? '#2c3f60' : '#17283f', color: '#d9e9ff', cursor: 'pointer' }}>Ver pendientes</button>
            <button onClick={() => setStateFilter('completed')} style={{ padding: '6px 10px', borderRadius: '8px', border: '1px solid #39654e', background: stateFilter === 'completed' ? '#244535' : '#173a2d', color: '#d5ffe6', cursor: 'pointer' }}>Ver completadas</button>
          </div>
          ...
          {!loading && !error && (
            <div style={{ display: 'grid', gap: '10px' }}>
              {visibleActivities.map((activity) => {
                ...
                return (
                  <div key={activity.udCode} style={{ border: '1px solid rgba(125, 212, 255, 0.22)', borderRadius: '10px', padding: '10px', background: 'rgba(10, 17, 30, 0.85)', display: 'grid', gap: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                      <strong style={{ color: '#d7efff' }}>{activity.udCode}</strong>
                      <span style={{ fontSize: '11px', color: activity.state === 'completed' ? '#9bf0be' : '#ffd0c8' }}>{activity.state === 'completed' ? 'Completada' : 'Pendiente'}</span>
                      {activityProgress[activity.udCode]?.status === 'submitted' && <span style={{ fontSize: '10px', color: '#ffe0aa', border: '1px solid rgba(255, 206, 120, 0.4)', borderRadius: '999px', padding: '2px 7px' }}>Enviada</span>}
                      {activityProgress[activity.udCode]?.status === 'reviewed' && <span style={{ fontSize: '10px', color: '#9de0ba', border: '1px solid rgba(123, 220, 160, 0.4)', borderRadius: '999px', padding: '2px 7px' }}>Revisada</span>}
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      <button onClick={() => toggleActivityState(activity.udCode)} style={{ padding: '5px 9px', borderRadius: '7px', border: '1px solid #355a7d', background: '#1a2f4a', color: '#dbecff', cursor: 'pointer', fontSize: '11px' }}>{activity.state === 'completed' ? 'Marcar pendiente' : 'Marcar completada'}</button>
                      <button onClick={() => setSelectedUdCode(activity.udCode)} style={{ padding: '5px 9px', borderRadius: '7px', border: '1px solid #335a76', background: selectedUdCode === activity.udCode ? '#2b4a60' : '#162a3d', color: '#d3eeff', cursor: 'pointer', fontSize: '11px' }}>Abrir ficha</button>
                      {firstResourceCode && <button onClick={() => navigate(`/dashboard?activity=${encodeURIComponent(activity.udCode)}&resourceCode=${encodeURIComponent(firstResourceCode)}`)} style={{ padding: '5px 9px', borderRadius: '7px', border: '1px solid #3f6e8f', background: '#18384f', color: '#b8ebff', cursor: 'pointer', fontSize: '11px' }}>Localizar recurso en mapa</button>}
                    </div>
                  </div>
                )
              })}

              {visibleActivities.length === 0 && (<div style={{ color: '#9cb8d2', fontSize: '12px' }}>No hay actividades en este estado.</div>)}

              {selectedTemplate && (
                <div style={{ border: '1px solid rgba(125, 212, 255, 0.25)', borderRadius: '10px', padding: '12px', background: 'rgba(8, 15, 27, 0.92)', display: 'grid', gap: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                    <strong style={{ color: '#d7efff' }}>{selectedTemplate.code} · {selectedTemplate.title}</strong>
                    <span style={{ color: '#9fd6ff', fontSize: '12px' }}>Progreso {selectedProgressStats.completed}/{selectedProgressStats.total} ({selectedProgressStats.percent}%)</span>
                  </div>
                  ...
                  {!!selectedTemplate.evaluationMethods?.length && (
                    <div style={{ display: 'grid', gap: '3px', fontSize: '11px', color: '#9dc2df' }}>
                      {selectedTemplate.evaluationMethods.map((method) => (<div key={`${selectedTemplate.code}-${method}`}>• {method}</div>))}
                    </div>
                  )}
                      ...
                      return (
                        <div key={step.id} style={{ border: '1px solid rgba(125, 212, 255, 0.18)', borderRadius: '8px', padding: '8px', display: 'grid', gap: '6px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                            <input type="checkbox" checked={!!stepState.completed} onChange={(e) => updateStepProgress(selectedUdCode, step.id, { completed: e.target.checked })} />
                            <strong style={{ color: '#d8f2ff', fontSize: '12px' }}>{step.title}</strong>
                          </div>
                          <div style={{ color: '#a6c9e5', fontSize: '11px' }}>{step.instruction}</div>

                          {/* ========== AQUÍ SE INTEGRA EL SUBCOMPONENTE ========== */}
                          {step.evidenceType === 'text' && (
                            <input
                              value={stepState.evidence || ''}
                              onChange={(e) => updateStepProgress(selectedUdCode, step.id, { evidence: e.target.value })}
                              placeholder="Nota/evidencia del paso"
                              style={{ padding: '6px', borderRadius: '7px', border: '1px solid #36577a', background: '#0f1d34', color: '#e4f4ff', fontSize: '11px' }}
                            />
                          )}

                          {step.evidenceType === 'image' && (
                            <ImageCropUploader
                              value={stepState.evidence}
                              onChange={(url) => updateStepProgress(selectedUdCode, step.id, { evidence: url })}
                              disabled={selectedProgress.status === 'submitted'}
                              udCode={selectedUdCode}
                              stepId={step.id}
                            />
                          )}
                  ...
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                    <button onClick={submitSimulation} style={{ padding: '7px 11px', borderRadius: '8px', border: '1px solid #3f6b8f', background: '#1a3f5d', color: '#d7efff', cursor: 'pointer', fontSize: '11px' }}>Simular envio y evaluacion</button>
                    <button onClick={markReviewedSimulation} style={{ padding: '7px 11px', borderRadius: '8px', border: '1px solid #4d7a65', background: '#1f4a3c', color: '#d7ffef', cursor: 'pointer', fontSize: '11px' }}>Marcar revisada</button>
                    <button onClick={exportSimulationReport} style={{ padding: '7px 11px', borderRadius: '8px', border: '1px solid #48678d', background: '#1a3554', color: '#d7ebff', cursor: 'pointer', fontSize: '11px' }}>Exportar informe JSON</button>
                    {selectedUdCode === 'UD01' && <button onClick={() => navigate('/sala-test-ud01')} style={{ padding: '7px 11px', borderRadius: '8px', border: '1px solid #3f6f8f', background: '#18384f', color: '#b8ebff', cursor: 'pointer', fontSize: '11px' }}>Abrir Sala Test UD01</button>}
                    {selectedProgress.submittedAt && (<span style={{ color: '#9de0ba', fontSize: '11px' }}>Ultimo envio: {new Date(selectedProgress.submittedAt).toLocaleString()} · Nota auto: {selectedProgress.autoScore ?? '-'} · Estado: {selectedProgress.status || 'draft'}</span>)}
                  </div>
                </div>
              )}
            </div>
