...
const project_task_model_1 = require("./project-task.model");
...
let ProjectsService = class ProjectsService {
    ...
    async resolveActor(email) {
        ...
        if (!user)
            return { id: null, name: normalizedEmail };
        return { id: user.id, name: user.name || user.email };
    }
    async resolveResource(resourceCode) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!tasks.length) {
                ...
                tasks.forEach((task, index) => {
                    doc
                        .fontSize(10)
                        .text(`${index + 1}. ${task.activityCode || '-'} | owner=${task.ownerUserEmail || '-'} | mesa=${task.mesaNum || '-'} | recurso=${task.resourceCode || '-'} | estado=${task.status || 'pending'}`);
                    if (task.notes) {
    ...
    async createProject(payload, actorEmail) {
    ...
    async updateProject(id, payload, actorEmail) {
    ...
    async removeProject(id, actorEmail) {
        const project = await this.findProject(id);
    ...
    async addProjectAttachment(id, fileUrl, actorEmail) {
        const project = await this.findProject(id);
    ...
    async createTask(projectId, payload, actorEmail) {
        const project = await this.findProject(projectId);
        const now = new Date().toISOString();
    ...
    async updateTask(taskId, payload, actorEmail) {
    ...
    async removeTask(taskId, actorEmail) {
        const task = await this.taskRepo.findOne({ where: { id: taskId } });
        if (!task)
            throw new common_1.NotFoundException(`Project task ${taskId} not found`);
